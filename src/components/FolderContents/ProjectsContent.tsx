...
import { ArrowUpRight, FolderGit2, Layers, Filter } from "lucide-react";

export function ProjectsContent() {
  const [selectedCategory, setSelectedCategory] = useState<string>("ALL");
  const [activeProject, setActiveProject] = useState<Project | null>(null);

  const categories = ["ALL", "Mobile", "Web & System", "Design & UI"];
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.08] pb-6">
        <div className="flex items-center gap-2 text-xs font-mono text-neutral-400">
          ...
          <span>INDEX // {filteredProjects.length} ITEMS CATALOGUED</span>
        </div>
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-mono transition-all ${
                selectedCategory === cat
                  ? "bg-purple-500/20 text-purple-200 border border-purple-400/40 shadow-[0_0_12px_rgba(168,85,247,0.25)] font-semibold"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
        {filteredProjects.map((project) => (
          <div
            ...
            data-cursor="VIEW"
            className="group relative flex flex-col justify-between rounded-2xl bg-neutral-900/60 hover:bg-neutral-900/90 border border-white/[0.08] hover:border-purple-400/40 p-5 sm:p-6 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.6)] cursor-pointer"
          >
            ...
            <div className="space-y-4 mb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-white/[0.04] group-hover:bg-purple-500 group-hover:text-white text-neutral-400 transition-all">
              ...
              {/* Title & Summary */}
              <div>
                <h3 className="text-xl sm:text-2xl font-serif text-white group-hover:text-purple-200 transition-colors mb-2 font-normal">
                  {project.title}
                </h3>
